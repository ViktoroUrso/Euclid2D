...
class Linear(Object)
    """ У всех линейных объектов должен быть метод для определения координаты у по имеющейся координате х и наоборот"""
    ''' Класс должен  поддерживать операции отражения, отображения, вращения, масштабирования, преобразования, афинные преобразования(?)'''
    length = None  # Full length of linear object

    def __len__(self): # property return full length of linear object (redefine for each subclass
        return length
...
A point p with Cartesian coordinates (px, py) is on the positive side of l, if a px + b py + c > 0, 
